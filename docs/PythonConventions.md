...
Document only the errors or exceptions that a method raises itself. Do
not document any errors or exceptions that are raised by other methods
that it calls. Those should be documented in the respective methods'
docstrings.
...
Docstrings will be parsed by a tool like mkdocs to generate html
documentation. Ensure that public modules have appropriate docstrings
with clear documentation to support this.

For code that has decision trees, provide graphs in mermaid format in
the docstring.

For code that has mathematical calculations, provide formulas in the
docstring in LaTeX format in the docstring.

## Using __main__

Expose the main script as a CLI command using "__main__".

In __main__, use argparse to parse arguments.

All arguments should have single-letter and long alternatives
e.g. "-y" and "--year".

The script should have a "--help" option with clear descriptions of
each argument and the default values when the arguments are optional.

Add a "--verbose" option that sets the logger to INFO level.

If the user provides wrong arguments, the script must produce the
help message to guide the user.

If the user provides no arguments, the script must default to the "-h"
option.
...
Typically, source files will go under an src directory and test files
under a tests directory.

Start new application script names with a verb e.g. "calc_important_values.py".

Start new test script names with "test_" prepended to the name of the
file that the unit test is for. This makes them discoverable by
pytest. e.g. "test_calc_important_values.py"
